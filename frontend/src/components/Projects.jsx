    ...
    >
      ...
      <div className="max-w-6xl mx-auto relative z-10">
        ...
        <div data-aos="fade-up" className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start w-full">
          ...
          {/* Left Panel: Project Selectors (4 cols) */}
          <div className="lg:col-span-4 flex flex-row lg:flex-col gap-4 overflow-x-auto lg:overflow-visible pb-4 lg:pb-0 w-full snap-x">
            {Object.values(projectsList).map((proj) => {
              const isActive = selectedProjectId === proj.id;
              return (
                <button
                  key={proj.id}
                  onClick={() => setSelectedProjectId(proj.id)}
                  className={`w-full min-w-[260px] snap-center text-left p-6 rounded-2xl border-2 transition-all duration-300 relative overflow-hidden ${
                    isActive
                      ? 'bg-black text-white border-black shadow-[6px_6px_0px_0px_rgba(255,42,42,1)]'
                      : 'bg-white text-zinc-800 border-zinc-200 hover:border-black hover:shadow-[4px_4px_0px_0px_rgba(0,0,0,0.15)]'
                  }`}
                >
                  <div className="flex justify-between items-center mb-2">
                    <span className={`text-[10px] font-black uppercase tracking-wider ${
                      isActive ? 'text-[#ff2a2a]' : 'text-zinc-500'
                    }`}>
                      {proj.duration}
                    </span>
                    {isActive && (
                  ...
                  <h3 className="text-xl font-heading font-black tracking-tight leading-tight">
                    {proj.title}
                  </h3>
          ...
          {/* Right Panel: Project Showcase (8 cols) */}
          <div className="lg:col-span-8 w-full">
            <AnimatePresence mode="wait">
              <motion.div
                ...
                className="w-full bg-white border-4 border-black rounded-[2rem] p-6 md:p-10 shadow-[8px_8px_0px_0px_rgba(0,0,0,1)] relative"
              >
                {/* Showcase Grid */}
                <div className="flex flex-col gap-8">
                  
                  {/* Mockup Frame */}
                  <div className="relative w-full aspect-[16/9] rounded-2xl overflow-hidden border-2 border-[#ff2a2a] bg-white flex items-center justify-center p-6 shadow-md">
                    <img 
                      src={currentProject.mockup} 
                      alt={currentProject.title} 
                      className={`select-none transition-transform duration-500 ${
                        currentProject.id === 'voiceai' ? 'max-h-full max-w-[60%] object-contain' : 'max-h-full max-w-[70%] object-contain'
                      }`}
                    />
                  </div>
                  ...
                  {/* Info Header */}
                  <div>
                    <h3 className="text-2xl md:text-3xl font-heading font-black text-black leading-none mb-1.5">
                    ...
                    {currentProject.id === 'voiceai' ? (
                      ...
                      <div>
                            ...
                            <button
                              ...
                              className={`px-3.5 py-1.5 rounded-xl text-xs font-black uppercase tracking-wider transition-all duration-300 ${
                                activePrimeDineTab === tabKey
                                  ? 'bg-[#ff2a2a] text-white shadow-[2px_2px_0px_0px_rgba(0,0,0,1)]'
                        ...
                        <motion.ul 
                          ...
                          className="space-y-3.5 text-zinc-700 text-sm leading-relaxed font-medium min-h-[160px]"
                        >
                  ...
                  {/* Actions / Buttons */}
                  <div className="flex flex-wrap items-center gap-3.5 border-t border-zinc-100 pt-6">
                    <a
                      href={currentProject.github}
                      target="_blank"
                      rel="noopener noreferrer"
                      className="inline-flex items-center gap-2 px-5 py-3 rounded-xl text-xs font-black uppercase tracking-wider bg-white hover:bg-black text-black hover:text-white border-2 border-black shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] hover:shadow-none transition-all duration-350"
                    >
                    ...
                    <a
                      ...
                      target="_blank"
                      rel="noopener noreferrer"
                      className="inline-flex items-center gap-2 px-5 py-3 rounded-xl text-xs font-black uppercase tracking-wider bg-[#ff2a2a] hover:bg-black text-white border-2 border-black shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] hover:shadow-none transition-all duration-350"
                    >
